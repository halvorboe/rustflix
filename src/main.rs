...
impl Handler for Server {
    ...
    fn on_message(&mut self, msg: Message) -> Result<()> {
        ...
        let mut file = File::open("data/0.b64").expect("Failed opening file!");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed generating string");
        self.out.send(Message::from(String::from_utf8(contents).unwrap()))
    }
...
//     // let paths = read_dir(String::from("videos")).unwrap();

//     // println!("[SEARCH] Found {:?} files...", paths);
...
//     //     println!("{:?}", filename);
//     //     let mut file = File::open(filename).expect("Failed opening file!");
//     //     let mut contents = Vec::new();
//     //     file.read_to_end(&mut contents).expect("Failed generating string");
//     //     let b = base64::encode(&contents);
//     //     // remove_file(format!("./data/{:?}.b64", filename));
//     //     let mut file = File::create(format!("./data/{:?}.b64", id)).expect("Failed crating file!");
...
//         move |msg| {
//             match msg {
//                 "0" => out.send(Message::from("A")),
//                 "1" => out.send(Message::from("A")),
//                 _ => out.send(Message::from("A"))
//             }
            
//         }
//     }).expect("Error setting up")
// }